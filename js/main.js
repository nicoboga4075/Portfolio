...
let appMessages = {
	...
	"success-mail" : {"fr": "Le message a été envoyé avec succès.'", "en": "The message was successfully sent."}
};

let tokenClient; // Generated by Google

let appSubSections = []; // Completed automatically

var checkpointSubSection;
...
function initPage(){   
  ...
  if (idPage === 'index'){
	...
	navLinks.forEach((navLink, index) => {
		navLink.href = `#${appSections[index]}`;
		navLink.addEventListener('click', function (event) {
			...
			saveHashSession(appSections[index]);
		});
	});
	...
	$('#navi a').each(function(){
		appSubSections.push($($(this).attr('href'))); 
	});

	$(window).scroll(function(){
		// Retains the value of the scroll top with the reference at the middle of the page
		var scrollTop = $(this).scrollTop() + ($(window).height()/2);
		for (let appSubSection of appSubSections) {
		  if (scrollTop > appSubSection.offset().top){
			var scrolled_id = appSubSection.attr('id');
		  }
		}
		if (scrolled_id !== checkpointSubSection) {
		   ...
		   iframeArticle.onload = () => {					 
				...
				.then(html => {													
					if(html.includes("It seems you've hit a broken link or the page has moved")){
						throw new Error(); // Catch directly to show error message when fetch redirects to 404 page
					}		
